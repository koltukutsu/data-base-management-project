...
CREATE
OR REPLACE VIEW company_view AS
SELECT
    DISTINCT companies.comp_name,
...
CREATE
OR REPLACE FUNCTION update_accepted_by_id() RETURNS TRIGGER AS $ $ BEGIN
UPDATE
    offers
SET
    accepted = 'FALSE',
...
CREATE
OR REPLACE TRIGGER user_delete_trigger
AFTER
    DELETE ON users FOR EACH ROW EXECUTE PROCEDURE update_accepted_by_id();
...
CREATE
OR REPLACE FUNCTION insert_into_balance() RETURNS TRIGGER AS $ $ BEGIN
INSERT INTO
...
AFTER
INSERT
    ON users FOR EACH ROW EXECUTE FUNCTION insert_into_balance();
...
CREATE
OR REPLACE FUNCTION getUserInfo(userId users.id % TYPE) RETURNS user_info AS $ $ DECLARE information user_info;

accepted_offer_count INT;

BEGIN
SELECT
    COUNT(*) INTO accepted_offer_count
FROM
    offers,
    organizations
WHERE
...
CREATE
OR REPLACE FUNCTION getProductsInStock(organizationName organizations.org_name % TYPE) RETURNS product_info [] AS $ $ DECLARE stock_cursor CURSOR FOR
SELECT
    products.id,
    product_name,
    price,
...
FROM
    ...
    organizations
WHERE
    products.product_type = organizations.id
    AND organizations.org_name = organizationName
GROUP BY
